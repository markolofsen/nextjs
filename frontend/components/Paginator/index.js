...
class Paginator extends Component {
	...
	componentDidMount() {
		...
		let pages_arr = range(1, Number(pagesTotal))
				pages_arr = pages_arr.filter(e => e !== Number(pagesCurrent))
		...
		return (
			<div>
				<div className={classes.pagination}>
					{paginator_visible &&
					<ul>
						{range(1, Number(pagesTotal)).map((item, index) => {
							return (
								<li key={index} data-selected={Number(pagesCurrent) == item}>
									<Link route={route}
				...
				<div style={{width: '100%'}}>
					...
					{preloader ? <Preloader /> : this.props.children}
...
Paginator.propTypes = {
	...
	pagesTotal: PropTypes.oneOfType([
		PropTypes.number.isRequired,
		PropTypes.string.isRequired
	]),
	pagesCurrent: PropTypes.oneOfType([
		PropTypes.number.isRequired,
		PropTypes.string.isRequired
	]),
	...
	loadMoreParams: PropTypes.object,
};
