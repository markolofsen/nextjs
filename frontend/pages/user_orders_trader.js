...
class UserOrdersPage extends Component {
  ...
  loadData = (pageNumber=false) => {
    ...
    new API(`users/{token}/orders/income/?page=${pageNumber_}`).get().then(res => {
  ...
  renderItems() {
    ...
    return (
      <div>
        {data.results.map((item, index) => (<OrderItem key={index} data={item} cancelOrder={this.cancelOrder} refreshData={this.refreshData} itemType='income' />))}
  ...
  render () {
    ...
    const custom_title = t('menu__orders-income')
