...
@translate('common')
@observer
class DialogTransferReceiveAddFeedback extends React.Component {
  state = {
    open: false,
    feedback: '',
    rating: 5,
  ...
  submitForm = (event) => {
    ...
    new API(`users/{token}/order/${data.id}/confirm_transfer_feedback/`).post(post).then(res => {
  ...
  renderOrderDetails = () => {
    ...
    return (
      <form onSubmit={this.submitForm} action='' className={classes.dialogWrapper}>
        <DialogTitle>
          {t('orders__dialog-transfer-feedback-title')}
        </DialogTitle>

        <DialogContent>
          <Typography variant="body2" gutterBottom>
            <span dangerouslySetInnerHTML={{
              __html: t('orders__dialog-transfer-feedback-message', {
...
export default withMobileDialog()(DialogTransferReceiveAddFeedback);
